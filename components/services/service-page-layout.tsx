...
interface ServicePageLayoutProps {
  ...
  subtitle: string
  description: string
  /** URL path for canonical + Service schema, e.g. /services/decorative-curbing */
  servicePath: string
  heroImage: string
  /** CSS `object-position` for portrait hero photos */
...
export function ServicePageLayout({
  ...
  return (
    <div className="pt-20">
        <ServiceJsonLd name={title} description={description} path={servicePath} />
        {/* Hero Section */}
        <section className="relative py-24 bg-[#0F172A] overflow-hidden">
          <div className="absolute inset-0 z-0 opacity-40">
            <Image
              src={heroImage}
              alt={`${title} — Screaming Eagle Curbing, La Crosse WI`}
              fill
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedServices.map((service) => (
                ...
                >
                  <Image
                    src={service.image}
                    alt={`${service.title} service — Screaming Eagle Curbing`}
                    fill
